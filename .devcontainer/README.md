...
**Note**: Only add this mount after creating the `.env` file, otherwise the container will fail to

start.

## Azure CLI Authentication

The dev container automatically mounts your local Azure CLI configuration (`~/.azure` or
`%USERPROFILE%\.azure`) to preserve authentication between container restarts.
...
If you encounter permission issues, you may need to adjust the `remoteUser` setting or use a
